...
const handleUpdateAccompaniment = (categoryId: string, itemId: string, change: number, accompaniments: any) => {
    ...
    // Lista de categorias com limite de quantidade POR ITEM
    const itemLimitedCategories = ["bebidas", "bolos", "complementos", "frios", "frutas"];

    // Só executa a validação ao TENTAR ADICIONAR um item
    if (change > 0) {
        // Regra para categorias com limite POR ITEM (ex: até 3 sucos, até 3 cafés, etc.)
        ...
        // Regra para a categoria com limite no TOTAL DE ITENS (apenas "Pães")
            ...
            if (currentCountInCategory >= absoluteLimit) {
                toast.warning(
                    <div>
                        ...
                        <p className="text-xs">Máximo de {absoluteLimit} unidades para {totalGuests} pessoa(s).</p>
                    </div>
                );
                return; // Impede a adição
            }
    ...
    // Se passou por todas as validações, atualiza o estado do pedido
    setOrderState((prev) => {
        ...
        if (newCount === 0) {
            // Remove o item do objeto se a contagem for zero para limpar o estado
            delete newAccompaniments[categoryId][itemId];
            // Se a categoria ficar vazia, remove a categoria também
