...
export const deactivateComanda = async (token: string) => {
    // CORREÇÃO: A lógica que desativa a comanda no banco de dados foi desativada.
    // Agora, a comanda só expirará por tempo ou por desativação manual no painel de admin.
    console.log(`Lógica de desativação por uso da comanda '${token}' foi pulada conforme solicitado.`);
    return;

    /*
    // CÓDIGO ORIGINAL (AGORA DESATIVADO)
    if (!token) return;
    try {
