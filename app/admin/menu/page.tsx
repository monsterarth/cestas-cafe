...
interface MenuItem {
  ...
  imageUrl?: string
  posicao: number
  sabores?: Flavor[] // Adicionado para pratos quentes
}
...
// Componente para um Sabor (dentro do modal)
function FlavorItem({ flavor, onEdit, onDelete }: { flavor: Flavor; onEdit: () => void; onDelete: () => void }) {
  return (
  ...
  return (
    <tr ref={setNodeRef} style={style} className="border-b bg-white">
      ...
      <td className="p-3 text-right">
        {isHotDish && (
          <Button variant="outline" size="sm" onClick={onManageFlavors}>
    ...
    <div ref={setNodeRef} style={style} className="bg-white rounded-xl shadow-sm border border-[#ADA192]">
      <div className="p-4 border-b border-[#ADA192] flex justify-between items-center bg-gray-50 rounded-t-xl">
        ...
        <div className="flex items-center gap-2">
          <Button variant="outline" size="sm" onClick={() => props.onAddItem(category.id)}>
            <Plus className="w-4 h-4 mr-1" />
            Adicionar Item
          </Button>
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => props.onEditCategory(category)}>
            <Edit className="w-4 h-4" />
          </Button>
          <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => props.onDeleteCategory(category.id)}>
            <Trash2 className="w-4 h-4" />
          </Button>
        </div>
      </div>
      <div className="p-2">
        <DndContext sensors={useSensors(useSensor(PointerSensor))} onDragEnd={props.onItemsDragEnd} collisionDetection={closestCenter}>
...
export default function MenuPage() {
        ...
        async (snapshot) => {
          const categoriesData = await Promise.all(snapshot.docs.map(async (categoryDoc) => {
            ...
            const items = await Promise.all(itemsSnapshot.docs.map(async (itemDoc) => {
              ...
              const flavorsSnapshot = await firestore.getDocs(
                firestore.collection(firestoreDb, "cardapio", categoryDoc.id, "itens", itemDoc.id, "sabores")
              )
  ...
  const handleSaveItem = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      if (item) {
        await firestore.updateDoc(firestore.doc(db, "cardapio", categoryId, "itens", item.id), itemData);
      } else {
        const itemsColl = firestore.collection(db, "cardapio", categoryId, "itens");
        const itemsSnapshot = await firestore.getDocs(itemsColl);
        await firestore.addDoc(itemsColl, { ...itemData, posicao: itemsSnapshot.size });
      }
  ...
  const handleDeleteItem = async (categoryId: string, itemId: string) => {
    if (!db) return;
    if (!confirm("Tem certeza que deseja excluir este item? Isso também removerá todos os seus sabores.")) return;
    try {
  ...
  const handleSaveFlavor = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const formData = new FormData(e.currentTarget)
    const flavorData = {
      nomeSabor: formData.get("nomeSabor") as string,
      disponivel: formData.get("disponivel") === "on",
    }
    const flavorRef = firestore.collection(db, "cardapio", flavorModal.categoryId, "itens", flavorModal.itemId, "sabores")
    try {
      if (currentFlavor) {
        await firestore.updateDoc(firestore.doc(flavorRef, currentFlavor.id), flavorData)
      } else {
        await firestore.addDoc(flavorRef, flavorData)
      }
      setCurrentFlavor(null);
      // Forçar recarregamento para ver a mudança
      const category = categories.find(c => c.id === flavorModal.categoryId);
      const item = category?.items.find(i => i.id === flavorModal.itemId);
      if(category && item) handleManageFlavors(category.id, item);
...
/>
      ...
      <Dialog open={flavorModal.open} onOpenChange={(open) => { if (!open) setFlavorModal({ open: false, flavors: [] })}}>
        <DialogContent className="max-w-3xl">
          <DialogHeader>
            <DialogTitle>Gerenciar Sabores para: {flavorModal.itemNome}</DialogTitle>
            <DialogDescription>Adicione, edite ou remova os sabores/preparos disponíveis para este prato quente.</DialogDescription>
          </DialogHeader>
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-4">
              <h4 className="font-semibold text-lg">{currentFlavor ? "Editar Sabor" : "Adicionar Novo Sabor"}</h4>
              <form onSubmit={handleSaveFlavor} className="space-y-4">
                ...
                <div className="flex items-center space-x-2"><input type="checkbox" id="disponivel" name="disponivel" defaultChecked={currentFlavor?.disponivel ?? true} /><Label htmlFor="disponivel">Disponível</Label></div>
                <div className="flex gap-2"><Button type="submit" className="bg-[#97A25F] hover:bg-[#97A25F]/90">Salvar Sabor</Button>{currentFlavor && <Button type="button" variant="ghost" onClick={() => setCurrentFlavor(null)}>Cancelar Edição</Button>}</div>
              </form>
            </div>
            <div className="space-y-3">
              <h4 className="font-semibold text-lg">Sabores Existentes</h4>
              <div className="space-y-2 max-h-64 overflow-y-auto pr-2">{flavorModal.flavors.map(flavor => <FlavorItem key={flavor.id} flavor={flavor} onEdit={() => setCurrentFlavor(flavor)} onDelete={() => handleDeleteFlavor(flavor.id)} />)}</div>
            </div>
          </div>
        </DialogContent>
      </Dialog>
